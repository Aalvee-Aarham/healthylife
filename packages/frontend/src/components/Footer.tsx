    ...
    <footer className="bg-slate-100 border-t border-slate-200 text-slate-600 text-xs py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
        ...
        <div className="space-y-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-lg font-extrabold text-slate-900">
              Healthy<span className="text-emerald-600">Life</span>
            </span>
          </div>
        ...
        <div className="space-y-2">
          <p className="text-slate-900 font-bold uppercase tracking-wider text-[11px]">Ecosystem</p>
          <ul className="space-y-1.5">
            <li><button onClick={() => onSelectTab('dashboard')} className="hover:text-emerald-600 transition-colors">Member Overview</button></li>
            <li><button onClick={() => onSelectTab('nutrition')} className="hover:text-emerald-600 transition-colors">Nutrition & AI Meals</button></li>
            <li><button onClick={() => onSelectTab('workouts')} className="hover:text-emerald-600 transition-colors">Gym & Workouts Log</button></li>
            <li><button onClick={() => onSelectTab('cycle')} className="hover:text-emerald-600 transition-colors">CycleSync™ Biological Phase</button></li>
            <li><button onClick={() => onSelectTab('ai-assistant')} className="hover:text-emerald-600 transition-colors">HealthyLife AI Advisor</button></li>
          </ul>
        </div>
        ...
        {/* Digest */}
        <div className="space-y-3">
          <p className="text-slate-900 font-bold uppercase tracking-wider text-[11px]">HealthyLife Digest</p>
          <p className="text-[11px] text-slate-500">Subscribe for weekly bio-hacking, nutrition research & AI coaching tips.</p>
          <div className="flex gap-2">
            <input
              type="email"
              placeholder="Enter email..."
              className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs text-slate-900 focus:outline-none focus:border-emerald-500 w-full"
            />
      ...
      <div className="max-w-7xl mx-auto pt-6 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4 text-[11px] text-slate-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <span>•</span>
          <span className="hover:text-slate-900 cursor-pointer">AI Security & Privacy</span>
        </div>
      </div>
    </footer>
  );
